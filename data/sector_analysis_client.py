...
class SectorAnalysisClient:
    ...
    def get_sector_companies(self, stock_code: str) -> dict[str, Any] | None:
        """종목코드로 업종 정보 조회"""
        try:
        ...
        try:
            ...
            for item in sector_data:
                item["relative_performance"] = (
                    "outperform" if item["return_percent"] > sector_avg_return else "underperform"
                )
                item["percentile_rank"] = (
                    sum(1 for r in returns if r < item["return_percent"]) / len(returns) * 100
                )
    ...
    def _estimate_market_cap(self, stock_code: str, current_price: float) -> float | None:
        """시가총액 추정 (간단한 추정치)"""
        try:
    ...
    def get_peer_comparison(self, stock_code: str, period_days: int = 90) -> dict[str, Any]:
        ...
        try:
            ...
            sorted_companies = sorted(
                sector_performance["company_details"], key=lambda x: x["return_percent"], reverse=True
            )
            ...
            return {
                ...
                "sector_benchmark": {
                    ...
                    "worst_performer": sector_performance["sector_statistics"]["worst_performer"][
                        "stock_code"
                    ],
                },
                "peer_comparison": [
                    {
                        ...
                        "relative_to_target": company["return_percent"]
                        - target_performance["return_percent"],
                    }
