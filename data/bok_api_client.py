...
class BOKAPIClient:
    ...
    def _make_request(
        self, stat_code: str, cycle: str = "D", start_date: str = None, end_date: str = None
    ) -> dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "StatisticSearch" in data and data["StatisticSearch"].get("row"):
                    ...
                    return {
                        "error": f"API 오류 - {data['RESULT'].get('MESSAGE', 'Unknown')}",
                        "status": "api_error",
                    }
                ...
                for item in result["StatisticSearch"]["row"]:
                    try:
                        ...
                        if date not in seen_dates and 1.0 <= rate_value <= 5.0:
                            rates.append(
                                {"date": date, "rate": rate_value, "unit": item.get("UNIT_NAME", "%")}
                            )
                            seen_dates.add(date)
    ...
    def get_exchange_rate(
        self, currency_code: str = "USD", start_date: str = None, end_date: str = None
    ) -> dict[str, Any]:
        """환율 조회
    ...
    def get_export_import_data(self, start_date: str = None, end_date: str = None) -> dict[str, Any]:
        ...
        try:
            ...
            if export_data and import_data:
                for exp, imp in zip(export_data, import_data, strict=False):
...
def get_macro_economic_indicators(indicators_list: list[str] = None) -> dict[str, Any]:
    ...
    try:
        ...
        if not base_rate_data.get("error"):
            ...
            indicators["base_interest_rate"] = {"error": base_rate_data.get("error"), "api_status": "failed"}
        ...
        if not usd_rate_data.get("error"):
            ...
            indicators["usd_exchange_rate"] = {"error": usd_rate_data.get("error"), "api_status": "failed"}
        ...
        if not gdp_data_result.get("error"):
            ...
            indicators["gdp"] = {"error": gdp_data_result.get("error"), "api_status": "failed"}
        ...
        if not export_data.get("error"):
            ...
            indicators["export_data"] = {"error": export_data.get("error"), "api_status": "failed"}
...
def get_sector_specific_indicators(sector: str = "manufacturing") -> dict[str, Any]:
    ...
    try:
        ...
        if sector == "manufacturing":
            ...
            # 전체 지표
            sector_data = {"comprehensive": get_macro_economic_indicators()}
