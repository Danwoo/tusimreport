...
def get_investor_trading_analysis_logic(
    stock_code: str, company_name: str = "Unknown", period_days: int = 20
) -> dict[str, Any]:
    """투자자별 매매 동향 분석 로직"""
    try:
        ...
        return convert_numpy_types(
            {"status": "success", "analysis_data": analysis_data, "data_source": "PyKRX"}
        )
...
def create_institutional_trading_agent():
    ...
    return create_react_agent(
        model=llm, tools=institutional_trading_tools, prompt=prompt, name="institutional_trading_expert"
    )
